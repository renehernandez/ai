...
- Agents must use `ax commit` instead of raw `git commit` when committing work. `ax commit` runs the local review gate before delegating to Git and does not remove the user's manual terminal escape hatch.
...
- Do not stage or commit local workflow artifacts into work-project repositories. Keep readiness reports, reviewer reports, delivery ledgers, screenshots, command proofs, rejected generated shapes, and private plan-support pointers in the thread or private workflow storage. Reusable AI repo workflow machinery, managed rules, skills, validators, runtime scripts, and regression fixtures may be committed only when that machinery is the feature being changed in this AI repo.
