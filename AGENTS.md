...
- Review-first plan workflows are the explicit exception to direct-main publishing. When `plan-orchestrator`, `plan-review`, or the approved plan workflow requires Nitro-reviewed stacked delivery, create the planning MR first, wait for the required Nitro-clean planning-review gate, and only then continue to stacked implementation sequencing.
