...
- After changing shared skill sources in this repo, refresh the installed runtime copies before treating the change as live. Use `pnpm agent-runtime skills update --profile <name>`, then confirm the active runtime surface with `pnpm agent-runtime skills status --profile <name>` or `pnpm agent-runtime validate --profile <name>`.
