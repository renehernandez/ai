...
function findBlockMatch(command: string): BlockMatch | undefined {
  ...
  if (absoluteMatch) {
    return {
      display: absoluteMatch[1].replace(
        /^.*\/node_modules\//u,
        "node_modules/",
      ),
...
function main(): void {
  ...
  if (!rawPayload.trim()) {
    writeDiagnostic(
      "No stdin payload received. Codex hooks normally provide JSON on stdin.",
    );
    return;
  }
  ...
  try {
    ...
    writeDiagnostic(
      `Could not parse Codex hook payload as JSON: ${(error as Error).message}`,
    );
    return;
  }
