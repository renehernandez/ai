...
- Package helper dependencies inside the current planning workflow skills that
  import repo-level helpers such as `../../../scripts/*` or sibling-skill
  helpers such as `../../openspec-tasks/scripts/*`.
- Remove AX/runtime guidance from non-`ax-cli` shared skills while preserving
...
- Affected skills: `openspec-tasks`, `plan-ready`, `plan-review`,
  `plan-orchestrator`, `plan-unit-sequencer`, `plan-unit-delivery`, other
  non-`ax-cli` shared skills with runtime-surface guidance, and `ax-cli`.
