...
**Reason**: Final artifact count now derives from the Plan/OpenSpec delivery shape; a planning MR is no longer a mandatory stack base.

**Migration**: Publish initial planning state in the POC, then publish one final MR for atomic work or one per top-level OpenSpec delivery unit.
