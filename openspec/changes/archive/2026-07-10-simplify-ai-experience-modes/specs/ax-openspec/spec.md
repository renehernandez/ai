...
**Reason**: First-time setup is one state handled by `ax openspec sync`; a separate install command creates an unnecessary mutation path.

**Migration**: Run `ax openspec sync` interactively or provide `--context-file` in headless execution.

### Requirement: Configured-project update reconciliation
**Reason**: Configured reconciliation is one state handled by `ax openspec sync`; a separate update command duplicates synchronization behavior.

**Migration**: Run `ax openspec sync` and keep the existing explicit config-review flags when reviewing inferred config changes.
