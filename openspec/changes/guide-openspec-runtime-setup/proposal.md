...
`agent-runtime` is currently exposed as a repo-local package script. That works
...
`agent-runtime openspec install` also currently runs upstream OpenSpec
...
- Add globally linked `agent-runtime` packaging and explicit source root,
  config path, target root, and executable path resolution.
- Add a read-only top-level `agent-runtime status` command for global runtime
...
- `agent-runtime-cli`: expose and resolve a globally linked runtime command.
- `agent-runtime-status`: report global runtime health and target project
  readiness.
...
- `package.json`
- any new executable wrapper used by the global `agent-runtime` bin
- `scripts/agent-runtime.ts`
- `agent-runtime.config.json`
- `tests/unit` and `tests/integration` runtime fixtures
- `AGENTS.md`, `skills/agent-runtime-cli/SKILL.md`, and possibly shared
