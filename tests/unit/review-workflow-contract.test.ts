...
test("orchestration guidance keeps review coverage explicit and bounded", () => {
  ...
  assert.match(review, /Closure is limited to enumerated repairs/i);
  assert.match(review, /exactly one\s+resolution per repair/i);
  ...
  assert.match(review, /do not re-prompt\s+solely for formatting/i);
  assert.match(review, /Return unresolved in-scope work automatically/i);
  ...
  assert.match(review, /per-type outcomes/);
  assert.match(review, /finding counts/);
  assert.match(review, /local\/hosted\/readiness status/);
  assert.match(review, /pre-commit hook owns the full local suite/i);
  assert.match(review, /outcome coverage, not reviewer count/i);
