---
name: start-project
description: Use when starting, scoping, mapping, kicking off, or preparing a new effort before planning or implementation, especially when the user wants local-first context, a project context pack, or tracker-ready intake.
...
portable Project Context Pack in chat, then recommend one follow-up workflow.
...
- already-approved plans ready for `plan-orchestrator`;
...
If a prompt mixes intake with breakdown or storage, return the Project Context
Pack only and recommend the requested breakdown or storage as a later follow-up.
...
5. Return one Project Context Pack in chat.
...
Remove empty sections only when they would be misleading. Keep the pack
portable Markdown so it can be pasted into a tracker or saved later.

## Hard Stops

Never include downstream breakdown or preview artifacts in the pack:
...
follow-up workflow and keep the pack at context-intake level.
...
The skill is working when these requests produce a context pack and preserve the
boundary:

- "Start this project and create tickets" returns the pack, then routes ticket
