...
Use after an OpenSpec change exists and before `plan-coordinate` delivers the
...
5. Classify manual, deployment, monitoring, and external-prerequisite tasks so
   `plan-coordinate` pauses with `needs_human_action` instead of sending them
   to `plan-to-pr`.
...
- GREEN: OpenSpec checkbox parsing now provides the task identity, line,
  heading, and manual/deliverable classification needed by `plan-coordinate`.
