...
| --- | --- | --- |
| `next_task` | User asks to continue, do the next delivery unit, or gives no broader completion target | One unchecked delivery unit |
| `complete_change` | User asks to finish, ship, complete, or push the whole OpenSpec change | All deliverable work items checked on target branch |
...
7. Select the first unchecked delivery unit in document order with
   `select-next-task --caller plan_orchestrator --goal complete_change` for
   orchestrator-driven runs.
8. Pass that delivery unit to `plan-unit-delivery`.
9. Require `plan-unit-delivery` to mark only nested work items from the
   selected delivery unit complete in one separate implementation PR/MR.
...
stacked delivery mode, selection base, and current `tasks.md` state: checked
delivery units, unchecked delivery units, and manual or external tasks.
...
base, selected delivery unit or completion state, artifact, and blocker or next
action.
...
the next unit. The stack tip `tasks.md` is cumulative: a stack tip for delivery
unit `1` has every deliverable work item in unit `1` checked. The target branch
...
Every new stacked unit must complete exactly one delivery unit relative to its
base branch, and each selected delivery unit must remain its own PR/MR in the
stack.
...
| `selected_unit_stale` | Target delivery-unit state changed | Rerun `plan-unit-sequencer` |
...
| `openspec_complete` | No unchecked deliverable work items remain | Report final delivery state |
...
| Treating a full-change request as one unit | Set `delivery_goal: complete_change` and loop until no unchecked delivery units remain |
...
| Combining multiple delivery units in one PR/MR | Split delivery so each unit has its own PR/MR |
