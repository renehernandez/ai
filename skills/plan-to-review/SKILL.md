...
10. Wait for routed automated feedback on the latest head:
    - Fullscript GitLab/Nitro: use `nitro-review-feedback`; do not request Nitro manually unless routing says explicit or the user asks.
...
plan_review_gate_ledger:
  ...
  automated_feedback:
    status: passed
    evidence: "Nitro latest-head feedback returned no findings"
  developer_review:
    status: passed
...
| Requesting Nitro manually when routing says automatic | Wait for automatic Nitro feedback unless explicit request is configured |
