...
This is a bounded Review specialist. It is read-only: it does not post, approve,
request review, resolve threads, publish, or merge. Missing `gh` access or
authentication returns a concrete hosted-verification gap, never a guessed
pass.
...
- Review resolves a GitHub PR and needs its complete hosted artifact state.
...
Return this shape so Review can normalize the hosted artifact consistently:
...
- REFACTOR: adapter output contract keeps artifact-host context separate from
  `diff-review` findings so Review can normalize the gate.

## Codex Review Boundary

Do not request, poll, normalize, or gate on Codex-authored PR review feedback.
GitHub review covers the PR artifact, diff, comments, threads, approvals, and
Actions state only. `codex-review-feedback` remains retired.
