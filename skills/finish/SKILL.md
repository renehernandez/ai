...
| `merge`, `ship`, `merge when green`, `add to merge queue` | Merge or queue after current gates |
...
final delivery, create one mergeable PR/MR per top-level delivery unit and no
...
artifact URL, target base, exact head, status, and findings to Review for
normalization. Route actionable implementation findings to the same Execute
owner. Every changed head reruns local Review, CI, and latest-head hosted gates.
...
state, and required tracker mapping. Readiness is not merge authority.

## Terminal Actions

Merge only under explicit merge language or activated project policy, after
current checks and approvals. Merge dependency chains bottom-to-top. After a
squash merge, verify the remote merged commit, retarget and restack the child,
and refresh changed-head gates before the next merge.
...
| Creating planning plus implementation MRs | Publish only the POC and final delivery-unit artifacts. |
